...
mod tests {
    ...
    fn test_cuts_lstsq() {
        ...
        // let U: Mat<f32> = faer::stats::UnitaryMat { dimension: nrows }.sample(rng);
        // let V: Mat<f32> = faer::stats::UnitaryMat { dimension: ncols }.sample(rng);

        // let mut S = Mat::<f32>::zeros(nrows, ncols);
        // for i in 0..Ord::min(nrows, ncols) {
        //     S[(i, i)] = 1.01f32.powi(i as i32);
        // }
        // let A = U * S * V.transpose();
        let A: Mat<f32> = faer::stats::StandardNormalMat { nrows, ncols }.sample(rng);
        ...
        while iter < 20000 {
            if how_full == blocksize {
                {
                    {
                        let mut A = Mat::<f64>::zeros(nrows * ncols, blocksize);
                        let B = Col::from_fn(nrows * ncols, |ij| {
                            let i = ij % nrows;
                            let j = ij / nrows;
                            -two_remainder[(i, j)] as f64
                        });

                        {
                            let S = S.rb().storage_as::<u8>();
                            let T = T.rb().storage_as::<u8>();

                            for p in 0..blocksize {
                                let tp = T.col_as_slice(p);
                                let sp = S.col_as_slice(p);

                                for j in 0..ncols {
                                    let t = (tp[j / 8] >> j % 8) & 1 == 1;
                                    for i in 0..nrows {
                                        let s = (sp[i / 8] >> i % 8) & 1 == 1;

                                        A[(i + nrows * j, p)] += if s == t { 1.0 } else { -1.0 };
                                    }
                                }
                            }
                        }
                        ...
                        let qr = A.col_piv_qr();
                        let sol = zipped!(qr.solve_lstsq(B)).map(|unzipped!(x)| *x as f32);
                        C.copy_from(&sol);
                    }
        ...
        let mut roundtrip = faer::Mat::<f32>::zeros(nrows, ncols);
        bitmagic::matmul::mat_tmat_f32(
            crate::MatMut::from_faer(roundtrip.as_mut()),
            S.rb(),
            T.rb(),
            &full_C,
        );
        dbg!((&roundtrip - &A).squared_norm_l2());
        dbg!(bf16_residual);
